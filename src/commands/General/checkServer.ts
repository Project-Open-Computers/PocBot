    ...
    async chatInputRun(interaction: CommandInteraction) {
        ...
        const serverInfoEmbed = new MessageEmbed() //
            ...
            .addField(
                ...
                serverData?.players?.online ? serverData?.players?.online.toString() : 'None',
                false
            )
            .addField(
                'MOTD: ',
                serverData?.motd?.clean ? serverData?.motd?.clean.join(' ') : 'Server is offline!',
                true
            )
            .addField(
                'Online Players: ',
                serverData?.players?.list?.length > 0 ? serverData.players.list.join(', \n') : 'None',
                false
            )
            .setColor('BLUE');
